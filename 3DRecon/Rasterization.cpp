...
void Rasterization::RasterTerrian(Cloth          & cloth,
                                  csf::PointCloud& pc,
                                  std::vector<double> & heightVal,std::string projpath) {
    ...
    for (int i = 0; i < cloth.getSize(); i++) {
        if((((i+1)/double(cloth.getSize()))*100)-j>0 || (i+1)==cloth.getSize())//为了避免输出太多
        {
            std::string  RunningStateMsg=BasicFunction::Convert2Json_new(200,"CSF_Filter is running",0.1+0.6*double(j)/double(100),"false");
            BasicFunction::Write2Text(projpath+"/RunningState_Log.txt",RunningStateMsg);
          
            std::cout<<int((i+1)/double(cloth.getSize())*100)<<" %\n";
            j+=5;
