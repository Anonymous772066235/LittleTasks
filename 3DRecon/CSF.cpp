...
void CSF::setProjPath(std::string projectPath)
{
...
void CSF::do_filtering(std::vector<int>& groundIndexes,
    ...
    Rasterization::RasterTerrian(cloth, point_cloud, cloth.getHeightvals(), projpath);

    //日志进度
    std::string RunningStateMsg=BasicFunction::Convert2Json_new(200,"CSF_Filter is running",0.7,"false");
    BasicFunction::Write2Text(projpath+"/RunningState_Log.txt",RunningStateMsg);
    ...
    for (int i = 0; i < params.interations; i++) {
        ...
        RunningStateMsg=BasicFunction::Convert2Json_new(200,"CSF_Filter is running",0.7+0.2*double(i+1)/double(params.interations),"false");
        BasicFunction::Write2Text(projpath+"/RunningState_Log.txt",RunningStateMsg);
